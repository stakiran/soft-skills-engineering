...
扱うためには名前、特に単語が必要だが、既存にはない――そういうわけで **造語は普通に発生します**。むしろ造語が発生しない方が不自然なくらいです。ですので、本節では「必要性」と書きましたが、むしろ「必然性」と書いた方が即しています。
...
この性質は直感的にはわかりにくいかもしれませんが、開発者の方は、自分が普段コーディングのときに付ける名前を思い浮かべてください。GitHub などで検索してみるといいでしょう。おそらくヒット件数は少ないでしょうし、GitHub なる広い世界であっても自分が書いたコードしかヒットしなかった、なんてこともありえます。



方向づけについては、概念の名前をつけることで豊富なニュアンス――特に開発者の意図を持たせるということです。QWINCS 
...
必ず名詞形にする。名前をつける
 理由は意味づけと再利用性のため
 意味づけ。名前付けは意思決定でもある。その概念をどう捉えているかを端的に表現できる。解釈を読み手に委ねるのではなく書き手がガイドするため。
 再利用性。名前をつけることで言及しやすくなり、扱えるようになる。「チャット以外にもコミュニケーションツールは 5 つある」よりも「QWINCS」の方が良い
 開発者であれば変数、関数、クラスやモジュール、ファイルと名前は日々つけているはずだし、その重要性もわかるはず。認知コストをどれだけ削減できるかは名前にかかっているし、ソフトスキルは人間が動かすものであり勝手には動かないので「名前はどうでもいい、動けばいい」は成立しない
